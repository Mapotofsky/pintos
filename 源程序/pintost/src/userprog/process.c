...
process_execute (const char *file_name) 
{
  ...
  char *hondo_name = NULL, *tmp = NULL;
  hondo_name = strtok_r(fn_copy, " ", &tmp);  // 截取命令（文件）名
  /*---------------my code-------------*/

  /* Create a new thread to execute FILE_NAME. */
  tid = thread_create (hondo_name, PRI_DEFAULT, start_process, file_name);
  if (tid == TID_ERROR) {
...
start_process (void *file_name_)
{
  ...
  /*---------------my code-------------*/
  success = load (file_name, &if_.eip, &if_.esp); // 需要输入文件名
  /*---------------my code-------------*/
  ...
  if (success) {
    int argc = 0;
    int argv[64];  // 最多64个参数
    char *token = strtok_r(fn_copy, " ", &tmp);
    while (token != NULL) {
